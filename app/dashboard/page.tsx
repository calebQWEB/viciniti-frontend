...
export default function DashboardPage() {
  ...
  const quickLinks = [
    {
      ...
      icon: <ShoppingBag className="w-4 h-4 text-primary-600" />,
    ...
    },
    {
      ...
      icon: <Wrench className="w-4 h-4 text-accent-600" />,
    ...
    },
    {
      ...
      icon: <PackageOpen className="w-4 h-4 text-blue-600" />,
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-3 mb-6">
        {quickLinks.map((link, index) => (
          <Link
            key={index}
            href={link.href}
            className="p-3 sm:p-4 rounded-2xl bg-white border border-gray-100 shadow-sm hover:shadow-md transition-all duration-200 flex items-center gap-3 group"
          >
            <div
              className={`w-10 h-10 ${link.color} rounded-xl flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform`}
            >
              {link.icon}
            </div>
      ...
      {/* Browse Section */}
      <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-4 sm:p-6 text-center">
